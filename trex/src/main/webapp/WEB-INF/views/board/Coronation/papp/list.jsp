...
<script type="text/javascript">menuclick('submenu3');</script>
	...
	<div id="content">
			...
			<table width="100%" class="table1 center" border="0" cellspacing="0" cellpadding="0">
				...
				<thead>
					<tr>
						<th scope="col" class="t_head">코드</th>
				...
				<tbody>
					<c:forEach var="PF_AppList" items="${PFAppList }">
						<tr role="row" class="jqgrow ui-row-ltr ui-widget-content">
						<td role="gridcell" style="text-align:center;" >${PF_AppList.pf_app_code }</td>
						<td role="gridcell" style="text-align:center;" >${PF_AppList.pf_app_num }</td>
						<td role="gridcell" >
							<a href="/board/Coronation/papp/detail/${PF_AppList.pf_app_code }" onclick="OpenWindow('detail?pf_app_code=${PF_AppList.pf_app_code }','','850','620')"
									 style='<c:if test="" >font-weight:bold;color:blue;</c:if>'>${PF_AppList.title }
							</a>
											   
						</td>
							<td role="gridcell" style="text-align:center;" >
							<a class="maninfo" rel="${PF_AppList.writer }" href="#" data-hasqtip="21">
								<i class="pink2 ace-icon fa fa-user" title="작성자"></i>															
								${PF_AppList.writer }
							</a>
							</td>
						<td role="gridcell" style="text-align:center;" >
							<fmt:formatDate value="${PF_AppList.regdate }" pattern="yyyy-MM-dd" />
						</td>
			...
			<div class="board_butt" style="margin: 10px 0;padding: 0 10px; text-align: right">
					...
					<a href="/board/Coronation/papp/regist">
